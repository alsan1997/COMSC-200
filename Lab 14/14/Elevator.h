...
class Elevator
{
  private:
    ...
    vector<Rider> rider; // to store passanger(rider)
  ...
  public:
    ...
    void setDestination(const Floor* f){toFloor = f;}// set toFloor pointer
    ...
    // lab 10 addition inline function
    bool hasRiders() const{return(!rider.empty());} // returns false if riders vector is empty
    ...
    int getAvailableSpace() const{return(capacity - getRiderCount());}; // return capacity minus size of riders vector
    ...
    // lab 10 addition non-inline function
